...
class I2PService {
  ...
  private async syncB32ToUser(): Promise<void> {
    ...
    // Phase-2 stub removed: the Electron I2CP layer now derives a real b32
    // locally from the privKey (see `electron/src/i2p/i2cp-socket-manager.ts`
    // `initialize()`) so the sentinel-guard is no longer needed. Any b32
    // that reaches here is a real wire-format address (`<52 chars>.b32.i2p`).
    try {
  ...
  private async initializeViaElectronI2P(config?: SAMConfig): Promise<I2PStatus> {
    ...
    try {
      ...
      // Phase-2 handshake complete: Electron I2CP layer now derives a real
      // b32 from the local privKey and confirms SessionStatus=Created before
      // returning. The previous sentinel-guard has been removed; any b32
      // that reaches this point is wire-format valid.
      this.currentStatus = {
        samConnected: true,
        samAvailable: true,
        address: result.b32Address,
